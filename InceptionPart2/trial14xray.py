...
def get_data_loader(dataset_main_path, batch_size):
    ...
    train_loader = torch.utils.data.DataLoader(train_data_new, batch_size=batch_size, shuffle=True, num_workers=6)
    val_loader = torch.utils.data.DataLoader(val_data, batch_size=batch_size, shuffle=True, num_workers=6)
...
def train(model, args):
    ...
    for epoch in range(args.epochs):
        ...
        model_path = get_model_name("inceptionv3_t3", args.batch_size,args.learning_rate, epoch, args.sm_model_dir)
    ...
    print("Graphs plotted...train() exited...")
...
if __name__ == '__main__':
    ...
    num_features_aux = inceptionv3.AuxLogits.fc.in_features
    
    inceptionv3.AuxLogits.fc = nn.Linear(in_features=num_features_aux, out_features=4)
    inceptionv3.aux_logits=False

    num_features_primary = inceptionv3.fc.in_features
    inceptionv3.fc = nn.Sequential(
                          nn.Linear(num_features_primary, 1024),
                          nn.BatchNorm1d(1024),
                          nn.Linear(1024 , 512),
                          nn.Dropout(0.5),
                          nn.Linear(512, 4))
    ...
    if use_cuda and torch.cuda.is_available():
      inceptionv3 = inceptionv3.to('cuda:0')
